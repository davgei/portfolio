...
def jacobian(joint_angles, joint_velocities):
    ...
    # ------------------------------------------
    # 2) Jw = rotasjonsdel, 3x3
    # Kolonne i = ledd-i sin rotasjonsakse i basekood.
    # ledd1 -> z0=(0,0,1)
    z0 = np.array([0, 0, 1])

    # ledd2 -> z1 = 3.kolonne i R0^1 -> (s1, -c1, 0) i "forenklet" oppsett
    z1 = np.array([s1, -c1, 0])

    # ledd3 -> z2, ofte parallel med z1 i forenklet CR
    z2 = np.array([s1, -c1, 0])
    ...
    # ------------------------------------------
    # 3) multipliser for å få cart. velocity (vx,vy,vz):
